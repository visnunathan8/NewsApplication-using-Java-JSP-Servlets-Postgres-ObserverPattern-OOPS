...
public class ByLine {
	...
	public static ByLine setAllByLineData(JSONObject byLineData) {
		...
		if (byLineData != null) {
			...
			if (byLineData.get("person") != null && !(byLineData.get("person").toString().isEmpty())) {
				JSONObject personData = (JSONObject) byLineData.get("person");
				Person person = Person.setAllPersonData(personData);
				byLine.setPerson(person);
			}
